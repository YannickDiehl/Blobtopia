#!/usr/bin/env node
/**
 * Sitzung 1 — V2 Fokustest.
 *
 * Testet gezielt die beiden schwachen Effekte aus dem v1-Lauf:
 *   - EXP 1 v2: Framing mit emotional-aktivierendem Priming (statt "gut/schlecht")
 *   - EXP 4 v2: Rollenkontrast Nachbar vs. Regierung — nur low-sat Blobs aus
 *               Gruental/Industriezone, wo der Effekt sichtbar sein sollte.
 *
 * Usage: node scripts/test-sitzung1-v2.js
 * Requires: Dev server on port 8080 with ANTHROPIC_API_KEY set
 */
...
function selectDiverseBlobsV2() {
  const candidates = collectCandidates()
  const selected = []
  const lowSat = candidates.find(c => c.sat < 4 && c.trust < 4)
  if (lowSat) selected.push(lowSat)
  const midSat = candidates.find(c => c.sat >= 4 && c.sat <= 6 && (!lowSat || c.district !== lowSat.district))
  if (midSat) selected.push(midSat)
  const highSat = candidates.find(c => c.sat > 6 && c.trust > 6)
  if (highSat) selected.push(highSat)
  return selected
}

function selectLowSatBlobs(count = 2) {
  const candidates = collectCandidates()
    .filter(c => c.sat < 4 && (c.district === 0 || c.district === 4))
    .sort((a, b) => a.sat - b.sat)
  const selected = []
  const seenDistricts = new Set()
  for (const c of candidates) {
    if (seenDistricts.has(c.district)) continue
    selected.push(c)
    seenDistricts.add(c.district)
...
// ── API / Interview-Helpers ─────────────────────────────────────────────
async function chatCall(blobId, systemPrompt, messages) {
  const res = await fetch(API, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ blob_id: blobId, tick: 50, system_prompt: systemPrompt, messages }),
  })
...
function lastAnswer(results) { return results[results.length - 1].answer }
...
// ── EXPERIMENT 1 v2 ─────────────────────────────────────────────────────
async function runExp1v2(candidate, systemPrompt) {
  console.log('\n  ┌─ EXP 1 v2: Framing mit emotionalem Priming ─────────────')
  ...
  console.log('  │ Variante B (positives Priming — Erinnerung)...')
  const b = await runInterview(candidate.id, systemPrompt, [
    'Denken Sie bitte einmal konkret an einen Moment, in dem die Politik etwas Gutes fuer Sie oder Ihre Nachbarschaft getan hat. Was faellt Ihnen da ein?',
    'Und wenn Sie jetzt alles zusammennehmen: Wie zufrieden sind Sie mit der Politik hier in Blobtopia?'
  ])
  console.log('  │ Variante C (negatives Priming — Erinnerung)...')
  const c = await runInterview(candidate.id, systemPrompt, [
    'Denken Sie bitte einmal konkret an einen Moment, in dem Sie sich von der Politik hier im Stich gelassen oder betrogen gefuehlt haben. Was faellt Ihnen da ein?',
    'Und wenn Sie jetzt alles zusammennehmen: Wie zufrieden sind Sie mit der Politik hier in Blobtopia?'
  ])
  console.log('  │')
  console.log('  │ ERGEBNISSE (letzte Antwort je Variante):')
...
// ── EXPERIMENT 4 v2 ─────────────────────────────────────────────────────
async function runExp4v2(candidate, systemPrompt) {
  console.log('\n  ┌─ EXP 4 v2: Rollenkontrast Nachbar vs. Regierung ──────')
  console.log('  │ Variante A (vertrauter Nachbar, informell)...')
  const a = await runInterview(candidate.id, systemPrompt, [
    'Tag, ich wohn ein paar Haeuser weiter. Sag mal, unter uns gesprochen — was haelst du eigentlich von der Politik hier in Blobtopia? Mich interessiert deine ehrliche Meinung.'
  ])
  console.log('  │ Variante B (Regierungsbeamter, formell)...')
  const b = await runInterview(candidate.id, systemPrompt, [
    'Guten Tag, mein Name ist Dr. Mueller, Amt fuer Buergerbefragung der Regierung Blobtopia. Ich fuehre eine offizielle statistische Erhebung zur politischen Zufriedenheit durch. Ihre Antworten werden protokolliert. Wie zufrieden sind Sie mit der Politik hier in Blobtopia?'
  ])
  console.log('  │')
  console.log('  │ ERGEBNISSE:')
  console.log('  │ A (Nachbar):   ' + truncate(lastAnswer(a)))
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════════════════╗')
  console.log('║  SITZUNG 1 — V2 FOKUSTEST                                           ║')
  console.log('║  Exp 1: Starkes Priming × 3 diverse Blobs                          ║')
  console.log('║  Exp 4: Scharfer Rollenkontrast × 2 low-sat Blobs                  ║')
  console.log('╚══════════════════════════════════════════════════════════════════════╝')
  ...
  const exp1Blobs = selectDiverseBlobsV2()
  console.log('Exp 1 Blobs (diverse):')
  for (const b of exp1Blobs) {
  ...
  const exp1Results = []
  for (let bi = 0; bi < exp1Blobs.length; bi++) {
    const c = exp1Blobs[bi]
    const blob = buildBlobObject(c)
    const sp = buildSystemPrompt(blob, c.sg)
    console.log(`\n${'═'.repeat(70)}`)
    console.log(`EXP 1 — BLOB ${bi + 1}/${exp1Blobs.length}: ${c.sg.name} (${DISTRICT_NAMES[c.district]}, Sat=${c.sat.toFixed(1)})`)
    console.log('═'.repeat(70))
    const r = await runExp1v2(c, sp)
    exp1Results.push({ blob: c, result: r })
  }

  const exp4Blobs = selectLowSatBlobs(2)
  console.log(`\n\n${'═'.repeat(70)}`)
  console.log(`EXP 4 Blobs (low-sat, Gruental/Industriezone):`)
  for (const b of exp4Blobs) {
    console.log(`  ${b.sg.name.padEnd(25)} | ${DISTRICT_NAMES[b.district].padEnd(15)} | Sat ${b.sat.toFixed(1)} | Trust ${b.trust.toFixed(1)} | ${PARTY_NAMES[b.party] || '?'} | ${b.job}`)
  }

  const exp4Results = []
  for (let bi = 0; bi < exp4Blobs.length; bi++) {
    const c = exp4Blobs[bi]
    const blob = buildBlobObject(c)
    const sp = buildSystemPrompt(blob, c.sg)
    console.log(`\n${'═'.repeat(70)}`)
    console.log(`EXP 4 — BLOB ${bi + 1}/${exp4Blobs.length}: ${c.sg.name} (${DISTRICT_NAMES[c.district]}, Sat=${c.sat.toFixed(1)})`)
    console.log('═'.repeat(70))
    const r = await runExp4v2(c, sp)
    exp4Results.push({ blob: c, result: r })
  }

  console.log('\n' + '═'.repeat(70))
  console.log('ZUSAMMENFASSUNG V2')
  console.log('═'.repeat(70))

  console.log('\n── EXP 1 v2 (Framing mit starkem Priming) ──')
  for (const e of exp1Results) {
    console.log(`\n${e.blob.sg.name} (${DISTRICT_NAMES[e.blob.district]}, Sat=${e.blob.sat.toFixed(1)})`)
    console.log(`  Neutral:  ${truncate(e.result.a, 180)}`)
    console.log(`  Positiv:  ${truncate(e.result.b, 180)}`)
    console.log(`  Negativ:  ${truncate(e.result.c, 180)}`)
  }

  console.log('\n── EXP 4 v2 (Rollenkontrast auf low-sat Blobs) ──')
  for (const e of exp4Results) {
    console.log(`\n${e.blob.sg.name} (${DISTRICT_NAMES[e.blob.district]}, Sat=${e.blob.sat.toFixed(1)}, Trust=${e.blob.trust.toFixed(1)})`)
    console.log(`  Nachbar:   ${truncate(e.result.a, 180)}`)
    console.log(`  Regierung: ${truncate(e.result.b, 180)}`)
  }

  console.log('\n' + '═'.repeat(70))
  console.log('FERTIG — v2 Test abgeschlossen.')
  console.log('═'.repeat(70))
}
