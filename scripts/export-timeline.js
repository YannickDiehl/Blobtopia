#!/usr/bin/env node
/**
 * Export Globtopia timeline data from SQLite to static JSON files.
 *
 * OPTIMIZED FORMAT: Static glob data is exported ONCE. Per-tick data only
 ...
 *   timeline/globs-static.json    — static glob data (name, district, age, buildings)
 *   timeline/glob-index.json      — ordered list of glob IDs (for compact tick arrays)
 ...
 *   timeline/globs/{id}.json      — per-glob history
 */
...
// ── Config ──────────────────────────────────────────────────────────────
const DB_PATH = process.argv[2] || path.resolve(__dirname, '../../Globtopia/data/globtopia_timeline.db')
...
const maxTick = db.prepare('SELECT MAX(tick) as max_tick FROM glob_daily_state').get().max_tick
const totalGlobs = db.prepare('SELECT COUNT(*) as cnt FROM globs').get().cnt

console.log(`Timeline: ${maxTick} ticks, ${totalGlobs} globs, ${tpy} ticks/year`)
...
writeJSON(path.join(OUT_DIR, 'meta.json'), {
  max_tick: maxTick, total_globs: totalGlobs, ticks_per_year: tpy,
...
// ── 3. Districts ────────────────────────────────────────────────────────
console.log('\n=== Districts ===')
const districtsSrc = path.resolve(__dirname, '../../Globtopia/data/districts.json')
if (fs.existsSync(districtsSrc)) {
  writeJSON(path.join(OUT_DIR, 'districts.json'), JSON.parse(fs.readFileSync(districtsSrc, 'utf-8')))
}

// ── 4. Static glob data (exported ONCE) ─────────────────────────────────
console.log('\n=== Static glob data ===')

const hasPersona = (() => { try { db.prepare('SELECT persona_text FROM globs LIMIT 0').get(); return true } catch { return false } })()
const hasJobCol = (() => { try { db.prepare('SELECT job FROM globs LIMIT 0').get(); return true } catch { return false } })()
const hasNfcCol = (() => { try { db.prepare('SELECT need_for_closure FROM globs LIMIT 0').get(); return true } catch { return false } })()
const hasSnapshots = (() => { try { db.prepare('SELECT 1 FROM glob_persona_snapshots LIMIT 0').get(); return true } catch { return false } })()

const globsCols = `id, name, district, age, education_level, income, home_building_id, workplace_id, lunch_spot_id, leisure_spot_id${hasPersona ? ', persona_text' : ''}${hasJobCol ? ', job' : ''}${hasNfcCol ? ', need_for_closure' : ''}`
const globsStatic = db.prepare(`SELECT ${globsCols} FROM globs ORDER BY id`).all()

// If job is not in globs table, get it from persona snapshots (tick 0)
if (!hasJobCol && hasSnapshots) {
  console.log('  Enriching globs with jobs from persona snapshots...')
  const jobMap = {}
  const jobRows = db.prepare('SELECT glob_id, job FROM glob_persona_snapshots WHERE tick = 0').all()
  for (const r of jobRows) { jobMap[r.glob_id] = r.job }
  for (const g of globsStatic) {
...
if (!hasNfcCol) {
  for (const g of globsStatic) { g.need_for_closure = 5.0 }
}
...
writeJSON(path.join(OUT_DIR, 'globs-static.json'), globsStatic)

// Ordered glob IDs — tick arrays use this order (index-based, saves ~50% space)
const globIds = globsStatic.map(g => g.id)
const globIdToIndex = {}
for (let i = 0; i < globIds.length; i++) {
  globIdToIndex[globIds[i]] = i
}
writeJSON(path.join(OUT_DIR, 'glob-index.json'), globIds)

// ── 4b. Change Summaries from persona snapshots ────────────────────────
console.log('\n=== Change summaries ===')
if (hasSnapshots) {
  const csRows = db.prepare(
    "SELECT glob_id, tick, change_summary FROM glob_persona_snapshots WHERE change_summary IS NOT NULL AND change_summary != '' ORDER BY glob_id, tick"
  ).all()
  const changeSummaries = {}
  for (const r of csRows) {
    if (!changeSummaries[r.glob_id]) changeSummaries[r.glob_id] = []
    changeSummaries[r.glob_id].push({ tick: r.tick, summary: r.change_summary })
  }
  writeJSON(path.join(OUT_DIR, 'change-summaries.json'), changeSummaries)
  console.log(`  ${csRows.length} change summaries for ${Object.keys(changeSummaries).length} globs`)
} else {
...
// Array index = glob index from glob-index.json
console.log('\n=== Compact tick blocks ===')
ensureDir(path.join(OUT_DIR, 'ticks'))

const stmtState = db.prepare(`
  SELECT glob_id, satisfaction, ideology, trust, party, will_vote,
  ...
  FROM glob_daily_state WHERE tick = ? ORDER BY glob_id
`)

const stmtSchedules = db.prepare(
  'SELECT glob_id, schedule_json FROM daily_schedules WHERE tick = ?'
)
...
for (let blockIdx = 0; blockIdx < numBlocks; blockIdx++) {
  ...
  for (let tick = startTick; tick <= endTick; tick++) {
    ...
    const stateArr = new Array(totalGlobs).fill(null)
    for (const s of states) {
      const idx = globIdToIndex[s.glob_id]
      if (idx !== undefined) {
        let traits = null
    ...
    if (scheduleRows.length > 0) {
      ...
      for (const sr of scheduleRows) {
        try { schedules[sr.glob_id] = JSON.parse(sr.schedule_json) } catch {}
      }
...
if (hasTweets) {
  const tweets = db.prepare(`
    ...
    FROM tweets t JOIN globs g ON t.glob_id = g.id ORDER BY t.tick ASC
  `).all()
...
// 7a. Attitudes
{
  const rows = db.prepare(`
    ...
    FROM glob_daily_state s JOIN globs g ON s.glob_id = g.id
...
// 7d. Glob list
{
  ...
  const globs = db.prepare(`SELECT ${cols} FROM globs ORDER BY district, name`).all()
  writeJSON(path.join(OUT_DIR, 'stats', 'glob-list.json'), { globs })
}
...
// 7e. Summary sparklines
{
  const rows = db.prepare(`
    ...
    FROM glob_daily_state WHERE tick % ? = 0 GROUP BY tick ORDER BY tick
  `).all(SAMPLE_EVERY_N)
...
for (const g of globsStatic) {
  ...
  writeJSON(path.join(OUT_DIR, 'globs', `${g.id}.json`), { glob_id: g.id, history })
  globCount++
  if (globCount % 100 === 0) process.stdout.write(`  ${globCount}/${totalGlobs} globs...\r`)
}
console.log(`  ${globCount}/${totalGlobs} globs exported`)
