...
use crate::creature::*;
use crate::math::{RunningStatistics, RunningStatisticsResults};
use crate::simulation::behaviours::{BasicReproductionBehaviour, StepBehaviour};
...
pub fn primer_behaviours() -> Vec<Box<dyn StepBehaviour>> {
    vec![
        ...
        Box::new(behaviours::EdgeHomeBehaviour {
            disabled_edges: vec![],
        }),
...
/// Behaviours for the Blobtopia political simulation.
/// Extends primer behaviours with social influence, voting, and protest.
pub fn blobtopia_behaviours() -> Vec<Box<dyn StepBehaviour>> {
    vec![
        Box::new(behaviours::BasicMoveBehaviour),
        Box::new(behaviours::WanderBehaviour),
        Box::new(behaviours::ScavengeBehaviour),
        ...
        Box::new(behaviours::StarveBehaviour),
        // Blobtopia-specific behaviours
        Box::new(behaviours::SocialInfluenceBehaviour {
            influence_strength: 0.05,
        }),
        Box::new(behaviours::VotingBehaviour::new()),
        Box::new(behaviours::ProtestBehaviour::new(0.6)),
    ]
}

pub fn use_preset(sim: &mut Simulation, preset: &PresetConfig) {
    let mut behaviours = match preset.name.as_str() {
        ...
        "blobtopia" => blobtopia_behaviours(),
        _ => primer_behaviours(),
    };
...
pub struct GenerationStatistics {
    ...
    pub speed: RunningStatisticsResults,
    pub size: RunningStatisticsResults,
    pub sense_range: RunningStatisticsResults,
    pub reach: RunningStatisticsResults,
    pub life_span: RunningStatisticsResults,
    pub age: RunningStatisticsResults,
    pub age_at_death: RunningStatisticsResults,
    pub food_balls_available: u32,
    pub food_balls_eaten: u32,
    pub creatures_eaten: u32,
    pub births: u32,
    pub deaths: u32,
}
...
pub struct SimulationStatistics {
    ...
    pub population: RunningStatisticsResults,
    pub speed: RunningStatisticsResults,
    pub size: RunningStatisticsResults,
    pub sense_range: RunningStatisticsResults,
    pub reach: RunningStatisticsResults,
...
pub fn get_statistics(sim: &Simulation, species_filter: Option<String>) -> SimulationStatistics {
    ...
    let generation_statistics = sim
        ...
        .map(|g| {
            ...
            g.creatures
                .iter()
                .filter(|c| species_filter.as_ref().map_or(true, |s| *s == c.species))
                .for_each(|c| {
                    count += 1;
                    ...
                    for eaten in &c.foods_eaten {
                        match eaten.2.as_str() {
                            "food_ball" => food_balls_eaten += 1,
                            "creature" => creatures_eaten += 1,
                            _ => {}
                        }
                    }
