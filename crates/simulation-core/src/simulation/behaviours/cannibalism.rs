...
impl CannibalismBehaviour {
    fn for_pred_prey_pair(
        &self,
        creatures: &mut Vec<Creature>,
        func: &mut dyn FnMut(&mut Creature, &mut Creature),
    ) {
        for i in 1..creatures.len() {
            if !creatures[i].is_active() {
                continue;
            }
            ...
            after
                ...
                .for_each(|prey| {
                    ...
                    if !predator.is_active() || !prey.is_active() {
                        return;
                    }
                    if predator.get_size() * self.size_ratio < prey.get_size() {
                        return;
                    }
...
impl StepBehaviour for CannibalismBehaviour {
    fn apply(&self, phase: Phase, generation: &mut Generation, sim: &Simulation) {
        if let Phase::ORIENT = phase {
            ...
            self.for_pred_prey_pair(&mut generation.creatures, &mut |predator, prey| {
                if prey.within_flee_distance(&predator.get_position()) {
                    ...
                    let rot = nalgebra::Rotation2::new(ang);
                ...
                predator.add_objective(Objective {
        ...
        if let Phase::ACT = phase {
            ...
            self.for_pred_prey_pair(&mut generation.creatures, &mut |predator, prey| {
                if !predator.can_reach(&prey.get_position()) {
                    return;
                }
