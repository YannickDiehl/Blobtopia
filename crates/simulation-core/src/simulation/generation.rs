...
use super::{
    behaviours::Phase,
    Creature, Food, FoodStatus, Simulation,
};
...
impl Generation {
    ...
    fn generate(
        ...
        food_locations: Vec<Point2<f64>>,
    ) -> Self {
        let food = food_locations.iter().map(|p| Food::new(*p)).collect();
    ...
    fn run_phase(&mut self, phase: Phase, sim: &Simulation) {
        sim.behaviours
            .iter()
            .for_each(|b| b.apply(phase, self, sim));
    }

    fn shuffle_creatures(&mut self, sim: &Simulation) {
        use rand::seq::SliceRandom;
        self.creatures.shuffle(&mut *sim.rng.lock().unwrap());
    }
