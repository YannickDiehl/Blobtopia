    ...
    fn apply(&self, phase: Phase, generation: &mut Generation, _sim: &Simulation) {
        if let Phase::PRE = phase {
            generation
                ...
                .filter(|c| c.is_alive())
                .for_each(|c| {
                    if let Some(o) = c.get_objective() {
                        c.status_history.push(o.reason);
                    }
                    c.reset_objective();
                });
        }
...
pub struct Simulation {
    ...
    callbacks: Vec<Box<dyn FnMut(&mut Simulation) + Send>>,
    pending_creatures: Option<Vec<Creature>>,
}
...
impl Simulation {
    ...
    pub fn add_generation_callback<F: 'static + Send>(&mut self, f: F)
    where
        F: FnMut(&mut Simulation),
    {
    ...
    /// Initialize creatures for tick-based mode (server).
    pub fn init_creatures(&mut self, creatures: Vec<Creature>) {
        self.pending_creatures = Some(creatures);
    }

    /// Advance the simulation by one generation (tick-based mode).
    pub fn advance(&mut self) -> Option<&Generation> {
        let creatures = self.pending_creatures.take().unwrap_or_else(|| {
            let last = self
                .generations
                .last()
                .expect("No generations and no pending creatures");
            self.exec_reproduction(&last.creatures)
        });
        ...
        let generation = Generation::new(self, creatures, self.generate_food());
        let has_living = generation.has_living_creatures();
    ...
    pub fn get_random_location(&self) -> Point2<f64> {
        self.stage
            .get_random_location(&mut self.rng.lock().unwrap())
    }
    ...
    pub fn get_random_gaussian(&self, mean: f64, var: f64) -> f64 {
        let normal = Normal::new(mean, var).unwrap();
        normal.sample(&mut *self.rng.lock().unwrap())
    }
    ...
    pub fn generate_food(&self) -> Vec<Point2<f64>> {
        ...
        (0..num_foods).map(|_n| self.get_random_location()).collect()
    }
}
