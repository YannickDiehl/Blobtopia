  ...
  , methods: {
    ...
    , dragEnd( pos ){
      ...
      if ( this.dragged ){
        ...
        // Tap: originale Intersection aus dragStart verwenden —
        // Kamera kann sich durch OrbitControls leicht verschoben haben,
        // ein neuer Raycast wäre unzuverlässig
        this.$emit('tap', { intersects: dragging.intersects, ray: dragging.ray })
      }
    }
    , drag( pos ){
      let intersects = this.raycast( pos )
      ...
      if ( this.dragging ){
        // Nur als Drag zählen, wenn Maus sich signifikant bewegt hat
        if ( this.dragStartPos ){
          let dx = pos.x - this.dragStartPos.x
          let dy = pos.y - this.dragStartPos.y
          if ( dx * dx + dy * dy > 0.0001 ){ // ~3-5px Schwelle
            this.dragged = true
          }
        }
